...
function openModalById(id) {
  const index = state.filtered.findIndex(function (product) {
    ...
    "Hola, me interesan estos productos del catálogo:\n" +
      state.pedido
        .map(function (product) {
          return "- " + product.name + " (Código: " + product.code + ")";
        })
        .join("\n")
  );
...
function bindEvents() {
  ...
  els.modalBack.addEventListener("click", function (event) {
    if (event.target === els.modalBack) {
      closeModalFn();
    }
  });
  ...
  els.catsDrawerBack.addEventListener("click", function (event) {
    if (event.target === els.catsDrawerBack) {
      closeDrawer(els.catsDrawerBack);
    }
  });

  els.cartDrawerBack.addEventListener("click", function (event) {
    if (event.target === els.cartDrawerBack) {
      closeDrawer(els.cartDrawerBack);
    }
  });
