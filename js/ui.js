...
import { fixImageUrl, escapeHtml, truncateText } from "./utils.js";
...
export function renderLogo(els) {
  ...
  footerText.textContent =
    CONFIG.footerText || ((CONFIG.brandName || "ASTREA") + " © 2026");
  ...
  img.onload = function () {
    ...
    const imgHtml =
      '<img class="logo-img" src="' + safeUrl + '" alt="' + safeAlt + '">';
...
function renderLogoFallback(logoWrap) {
  const title = String(
    CONFIG.brandTitle || CONFIG.brandName || "ASTREA Catálogo"
  ).trim();

  logoWrap.innerHTML =
    '<div class="logo-fallback">' + escapeHtml(title) + "</div>";
}
...
export function renderCategoryButtons(els, onSelect) {
  ...
  ["Todas"].concat(categories).forEach(function (label) {
    els.categoryButtons.appendChild(makeButton(label));
    els.categoryButtonsMobile.appendChild(makeButton(label));
  });
}
  ...
  list.forEach(function (product) {
    ...
    card.innerHTML = `
      <div class="product-top">
        <div class="product-meta">
          <span class="icon-dot"></span>
          <span class="product-category">${escapeHtml(product.category || "Sin categoría")}</span>
        </div>
      ...
      <div class="product-figure">
        ${
          product.img
            ...
            : `<div class="muted">${escapeHtml(product.code || product.name)}</div>`
        }
      </div>

      <p class="product-desc">${escapeHtml(
        truncateText(product.desc || "Sin descripción.", 120)
      )}</p>
    ...
    card.addEventListener("click", function (event) {
      if (
        ...
        event.target.classList.contains("btn-detail")
      ) {
        handlers.openModalById(product.id);
      }
    });
...
export function renderModalGallery(els, product, activeImage, onThumbClick) {
  ...
  gallery.forEach(function (url) {
    ...
    button.type = "button";
    button.className = "modal-thumb" + (url === activeImage ? " active" : "");
    button.innerHTML =
    ...
    els.mGallery.appendChild(button);
  });
}
...
export function renderCartTo(container, onRemove) {
  ...
  if (!state.pedido.length) {
    container.innerHTML =
      '<div class="cart-empty">No hay productos seleccionados todavía.</div>';
    return;
  }
