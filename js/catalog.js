...
import { fixImageUrl, splitGallery, uniq } from "./utils.js";
...
export function applyFilters(query) {
  ...
  state.filtered = state.products.filter(function (product) {
    ...
    return inCategory && haystack.indexOf(q) !== -1;
  });
...
export function addToCart(id) {
  ...
  const exists = state.pedido.find(function (product) {
    return product.id === id;
  });
...
export function removeFromCart(id) {
  state.pedido = state.pedido.filter(function (product) {
    return product.id !== id;
  });
...
export function openModalState(product, index) {
  ...
  state.currentModalImage =
    product.img || (product.gallery && product.gallery[0]) || "";
}
...
export function getModalSibling(direction) {
  if (state.currentModalIndex < 0 || !state.filtered.length) {
    return null;
  }
